...
public class NXJournal
{
  public static void Main(string[] args)
  {
    ...
    // 步骤 1: new_document
    m.Tool_NewDocument("test_final");
    // 步骤 2: block
    m.Tool_Block(0.0, 0.0, 0.0, 100.0, 80.0, 60.0);

    m.Tool_SaveModel(@"E:/NX/Lang-to-NX-MCP/output\\models\\test_final.prt");
  }
}
...
public class NXModeler
{
    ...
    public void Tool_CreateSketchOnFace(int faceIndex)
  {
    try
    {
      ...
      SketchInPlaceBuilder sb = wp.Sketches.CreateSketchInPlaceBuilder(curSketch);
    ...
    public void Tool_Thread(string cylinderRef, string threadType, double length)
  {
    try
    {
      if (curBody == null) { Console.WriteLine("[!] Thread: no body"); return; }
      var builder = wp.Features.CreateThreadBuilder(null);
      if (!string.IsNullOrEmpty(cylinderRef))
      {
        try {
          NXObject obj = wp.Features.FindObject(cylinderRef);
          if (obj is Face) builder.CylindricalFace = (Face)obj;
        } catch { }
      }
      string tp = (threadType ?? "metric").ToLower();
      builder.ThreadType = tp.Contains("unified")
          ? ThreadBuilder.TypeOption.Unified : ThreadBuilder.TypeOption.Metric;
  ...
  public void Tool_BoundedPlane(string[] boundaryCurves)
  {
    try
    {
      ...
      foreach (string refName in boundaryCurves)
      {
        ...
        if (obj is ICurve)
        {
          ICurve cv = (ICurve)obj;
          SelectionIntentRuleOptions ro = wp.ScRuleFactory.CreateRuleOptions();
          ro.SetSelectedFromInactive(false);
          CurveDumbRule rule = wp.ScRuleFactory.CreateRuleCurveDumb(new ICurve[]{cv});
          ro.Dispose();
  ...
  public void Tool_TrimSurface(string faceRef, string[] boundaryCurves)
  {
    try
    {
      ...
      var builder = wp.Features.CreateTrimSurfaceBuilder(null);
      builder.TargetBodies.Add(curBody);
      if (boundaryCurves != null)
      {
        foreach (string refName in boundaryCurves)
        {
          try { NXObject obj = wp.Features.FindObject(refName); if (obj != null) builder.BoundaryObjects.Add(obj); }
          catch { }
        }
      }
      Feature f = builder.CommitFeature(); builder.Destroy();
...
// ═══════════════════════════════════════════════════════════════════════
    ...
    public void Tool_ReplaceFace(int[] targetFaces, int[] toolFaces)
  {
    try
    {
      ...
      SelectionIntentRuleOptions ro = wp.ScRuleFactory.CreateRuleOptions();
      ro.SetSelectedFromInactive(false);
      FaceDumbRule targetRule = wp.ScRuleFactory.CreateRuleFaceDumb(tgt.ToArray(), ro);
      ScCollector targetColl = wp.ScCollectors.CreateCollector();
      targetColl.ReplaceRules(new SelectionIntentRule[]{targetRule}, false);
      builder.FaceToReplace = targetColl;
      FaceDumbRule toolRule = wp.ScRuleFactory.CreateRuleFaceDumb(tool.ToArray(), ro);
      ScCollector toolColl = wp.ScCollectors.CreateCollector();
      toolColl.ReplaceRules(new SelectionIntentRule[]{toolRule}, false);
      builder.ReplacementFaces = toolColl;
      ro.Dispose();
      NXObject f = builder.Commit(); builder.Destroy();
      if (f is Feature) _lastFeature = (Feature)f;
    ...
    public void Tool_ResizeFace(int[] faceIndices, double newRadius)
  {
    try
    {
      ...
      SelectionIntentRuleOptions ro = wp.ScRuleFactory.CreateRuleOptions();
      ro.SetSelectedFromInactive(false);
      FaceDumbRule rule = wp.ScRuleFactory.CreateRuleFaceDumb(sel.ToArray(), ro);
      ScCollector coll = wp.ScCollectors.CreateCollector();
      coll.ReplaceRules(new SelectionIntentRule[]{rule}, false);
      ro.Dispose();
      builder.Face = coll;
      builder.Diameter.RightHandSide = (newRadius * 2.0).ToString();
      Feature f = builder.CommitFeature(); builder.Destroy();
      Console.WriteLine("[OK] ResizeFace: r={0}, d={1}, {2} faces", newRadius, newRadius*2, sel.Count);
    }
    ...
    public void Tool_Scale(double factor)
  {
    try
    {
      ...
      var builder = wp.Features.CreateScaleBodyBuilder(null);
      builder.Body = curBody;
      builder.ScaleFactor.RightHandSide = factor.ToString();
      builder.ScalePoint = wp.Points.CreatePoint(new Point3d(0, 0, 0));
      Feature f = builder.CommitFeature(); builder.Destroy();
      _lastFeature = f;
      Console.WriteLine("[OK] Scale: factor={0}", factor);
    }
